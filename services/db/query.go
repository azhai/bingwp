...
func GetLatestDailyRows(limit, offset int) []*WallDaily {
	...
	if err == nil {
		err = dbutil.ScanToList(&dailyRows, rows)
	}
	if err != nil {
		logutil.Error(err)
	}
...
func GetMonthDailyRows(monthBegin, nextBegin time.Time) []*WallDaily {
	...
	rows, err := DB().Query(query, date1, date2)
	var dailyRows []*WallDaily
	if err == nil {
		err = dbutil.ScanToList(&dailyRows, rows)
	}
...
func GetDailyImages(dailyRows []*WallDaily) []*WallDaily {
	...
	rows, err := DB().FlattenQuery(nil, query, ids)
	var imageRows = make(map[int64]map[string]*WallImage)
	if err == nil {
		err = dbutil.ScanToSecondary(imageRows, rows)
	}
	if err != nil {
		logutil.Error(err)
	}
...
func GetDailyNotes(dailyRows []*WallDaily) []*WallDaily {
	...
	if err == nil {
		err = dbutil.ScanToSecondary(noteRows, rows)
	}
	if err != nil {
		logutil.Error(err)
	}
...
func InsertDailyRows(dailyRows []*WallDaily, dates []any) (int, error) {
	...
	rows, err := DB().FlattenQuery(nil, query, dates)
	var existRows = make(map[string]*WallDaily)
	if err == nil {
		err = dbutil.ScanToUnique(existRows, rows)
	}
	if err != nil {
		logutil.Error(err)
	}
	...
	for _, row := range dailyRows {
		...
		if _, ok := existRows[bingDate]; !ok {
			...
			_, err = DB().UpdateRow(row, changes)
		}
	}
	return dbutil.InsertBatch(DB(), newbieRows)
}
