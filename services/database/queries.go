...
func GetMonthDailyRows(monthBegin, nextBegin time.Time) []*WallDaily {
	...
	rows, err := GetDB().Query(sql, date1, date2)
...
func GetDailyImages(dailyRows []*WallDaily) []*WallDaily {
	...
	rows, err := GetDB().Query(sql + " AND file_name LIKE 'thumb/%' ORDER BY daily_id")
	var thumbRows = make(map[int64]*WallImage)
	if err == nil {
		err = ScanToUnique(thumbRows, rows)
	}
	CheckErr(err)
	rows, err = GetDB().Query(sql + " AND file_name LIKE 'image/%' ORDER BY daily_id")
